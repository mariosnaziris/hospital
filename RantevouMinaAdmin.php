...
$sql_query = 'SELECT Name,Surname,Imerominia,Date_Time_id, Time FROM patients INNER JOIN appointment USING(Patient_id)INNER JOIN schedule USING (Date_Time_id) INNER JOIN hour USING(Time_id) WHERE  Imerominia>="'.$today.'" AND Imerominia<="'.date("Y-m-31").'" AND Doctor_id="'.$row_clinic1['Doctor_id'].'" ORDER BY Imerominia,Time';
...
<br/>
<h6><span class="style1">Ραντεβού Μήνα Ιατρού</span></h6>
<br/>

<form name="form1" method="post" action="RantevouMinaAdmin.php?id=<?php echo $row_clinic1['Doctor_id'];?>&sp=<?php echo $sp;?> ">
