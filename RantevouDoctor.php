...
<title>Ραντεβού Ημέρας Ιατρού</title>
		...
		if(isset($_POST['delete']) &&  ($_POST['delete']=='Διαγραφή') ){
				foreach($_POST as $key => $value){
					if(strncmp($key,"Date_Time_id_",13)==0){
						...
						$sql1='DELETE FROM appointment WHERE Date_Time_id="'.$value.'" AND Doctor_id="'.$_GET['id'].'"';	
...
$sql_query = 'SELECT Name,Surname,Imerominia,Date_Time_id, Time FROM patients INNER JOIN appointment USING(Patient_id)INNER JOIN schedule USING (Date_Time_id) INNER JOIN hour USING(Time_id) WHERE  Imerominia="'.$today.'" AND Doctor_id="'.$row_clinic1['Doctor_id'].'" ORDER BY Imerominia,Time';
...
<br/>
<h6><span class="style1">Ραντεβού Ημέρας Ιατρού</span></h6>
<br/>

<form name="form1" method="post" action="RantevouDoctor.php?id=<?php echo $row_clinic1['Doctor_id'];?>&sp=<?php echo $sp;?> ">
